...
| メインウィンドウ `Closing` | `Window.*` を現在値で更新して `SaveAsync()`。**書き終わるまで閉じるのを止める** |

保存の契機が複数あるので、`SaveAsync()` の書き込みは `SemaphoreSlim` で直列化する。

`Closing` は `await` を待ってくれないので、そのまま非同期に保存するとプロセスが先に落ちて
書き込みが間に合わない。一度 `e.Cancel = true` で閉じるのを止め、保存し終えてから
あらためて `Close()` する。
...
| キー | 型 | 既定値 | 適用 | 内容 |
|---|---|---|---|---|
| `RefreshIntervalSeconds` | int | `60` | ✔ | 自動更新間隔。`60` / `120` / `300` / `0`（更新しない）のみ。下限 60 秒 |
| `StartupState` | enum | `Normal` | ✔ | `Normal` / `Minimized` / `Tray` |
| `MinimizeButtonAction` | enum | `KeepInTaskbar` | ✔ | `KeepInTaskbar` / `MinimizeToTray` |

旧 `ActiveFilterIndex` は削除した。**ビューは起動時つねに「すべて」**で、前回の選択は復元しない。
...
|---|---|
| `0` 以下 | `0`（更新しない） |
| 正の値 | `60` / `120` / `300` のうち最も近い値。等距離なら短い方 |
...
| `X` | double? | `null` | ✔ | 前回の位置。`null` は未保存で、その場合は OS に任せる |
| `Y` | double? | `null` | ✔ | 同上 |
| `SplitterPosition` | double | `150` | ✔ | 詳細パネルの高さ。起動時に復元（`> 0` のとき）、終了時に保存 |

保存するのは `RestoreBounds`（最大化・最小化を解いた状態の位置と大きさ）。
最大化したまま終了しても、次に開いたとき元の場所へ戻せる。

`X` / `Y` が「未保存」を `null` で表すのは、負の値も正しい位置になりうるため
（左側に置いたサブモニタの座標は負になる）。`-1` のような番兵は使わない。

### 画面外の位置を戻さない

復元する前に `WindowPlacement.IsOnScreen` で確かめ、画面に残る量が
幅 120 / 高さ 40 に満たなければ復元をやめて OS の既定位置に任せる。
モニタを外した後などに掴めない場所へ出ると、設定ファイルを手で消すまで直せなくなるため。
...
一覧を持つページのうち、まだカード行へ移していないもの（自動ダウンロードルール）の
コードビハインドは「リスト選択 → テキストボックスへ反映 → 入力 → 複製のオブジェクトへ書き戻し」
...
カード行へ移したページ（全般のプレイヤー・Yellow Pages）はこの形を取らない。
一覧の行は表示専用で、編集は「⋯ → 編集」で開くダイアログが受け持つ。
ダイアログは複製を受け取り、OK のときだけ元のオブジェクトへ書き戻す
（`_loading` フラグが要らなくなり、追加をやめることもできる）。

## 6. 設定 UI の部品

| 部品 | 置き場所 | 役割 |
|---|---|---|
| `SettingCard` | `Views/Controls/SettingCard.cs` | 1 設定 = 1 行。アイコン / 見出し / 説明 / 状態 / 右のコントロール |
| `SettingsDraft` | `Settings/SettingsDraft.cs` | ダイアログが編集する複製。OK で本体へ書き戻す |
| 見た目の定義 | `Themes/Settings.xaml` | カード・左ナビ・トグルスイッチ・グループ枠のスタイル |

`SettingCard` の文字は 3 段。空の段は行ごと消える。

| 段 | プロパティ | 内容 |
|---|---|---|
| 1 | `Title` | 設定の名前 |
| 2 | `Description` | 設定の値・補足（`IsDescriptionMonospace` で等幅） |
| 3 | `Status` | 実行時に変わる状態。`IsStatusError` で赤字 |

`Description` と `Status` を分けているのは、「自分が設定した内容」と「いま起きていること」が
同じ見た目だと読み分けられないため。
