...
**無い。** 取得した行は 19 フィールド未満と `Id` 空を除いてすべて一覧へ渡す。

以前は YP ごとにビットレート上下限とコーデックの正規表現で絞り込めたが、削除した
（[design/decisions.md](../design/decisions.md#なぜ-yp-ごとの絞り込みをやめたか)）。
絞り込みはタグとビューが受け持つ（[matching.md](matching.md)）。
